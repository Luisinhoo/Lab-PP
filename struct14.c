/*
   Atividade 14: Faça um programa que controle o fluxo de voos nos aeroportos de um país. Com V = 5 (voos)
                 e A = 5 (aeroportos) e:
...
int main()
{
  ...
  for( i = 0; i < 5; i++)
  {
    printf("Digite o código do aeroporto de origem do %dº voo: ", i + 1);
    scanf("%d", &voos[i].codigoOrigem);
    
    printf("Digite o código do aeroporto de destino do %dº voo: ", i + 1);
  ...
  for(i = 0; i < 5; i++)
  {
    printf("Digite o código do %dº aeroporto: ", i + 1);
    scanf("%d", &a[i].codigo);
